...
const madlibs1 = ['Big', 'Small', 'Crooked', 'Twisted', 'Rigid', 'Long', 'Short'];
const madlibs2 = ['red', 'orange', 'yellow', 'green', 'blue', 'indigo', 'violet'];
const madlibs3 = ['cookie', 'brownie', 'pretzel', 'pizza', 'drumstick', 'noodle', 'pie'];


for (let i = 0; i < 101; i++){
	let randoProductName = madlibs1[Math.floor(Math.random() * 6.99)] + ' ' + madlibs2[Math.floor(Math.random() * 6.99)] + ' ' + madlibs3[Math.floor(Math.random() * 6.99)];

	for (let j = 0; j < products.length; j++){
		if (products[j].name === randoProductName) { randoProductName += i; }
	}
