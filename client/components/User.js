    ...
    return (
      ...
      <div id="single-user">
        <div key={thisUser.id} className="single-user-entry">
          <h2 className="user-name">Welcome, {thisUser.name}! What a purrfect day!</h2>
          <div><Link to={`/user/edit/${thisUser.id}`}><button type="button" className="edit-button" name={thisUser.id} >Edit Profile</button></Link></div>
        </div>
      </div>
    )
  }
